  ...
  constructor(scope: Construct, id: string, props?: ControlPlaneStackProps) {
    ...
    const assetsBucket = new s3.Bucket(this, "AssetsBucket", {
      ...
      lifecycleRules: [
        ...
        {
          // **削除を本当の削除にする**。バケットはバージョニング有効なので、`DeleteObject`
          // (VersionId 無し) は削除マーカーを付けるだけで実体は旧バージョンとして残り続ける。
          // これが無いと「イベントを削除したのに録画と字幕が S3 に無期限で残る」= 削除請求に
          // 応えられない、かつ課金され続ける。
          //
          // ここに置くオブジェクトは上書きされない (録画は UUID 名で 1 回だけ書かれる)。
          // つまり旧バージョンは削除後にしか生まれず、消しても失うものが無い。
          // 7 日なのは、誤って消したときに気づいて戻せる余地を残すため。
