...
> ⚠️ **2026-09-18 追記: このファイルは 3 ヶ月前の引継ぎで、§2 の「次にやるべきタスク」は
> ほぼ消化済み。** 現在地は [`NEXT_WORK.md`](./NEXT_WORK.md) を見ること。
>
> | §   | タスク                  | 2026-09-18 時点                                                                          |
> | --- | ----------------------- | ---------------------------------------------------------------------------------------- |
> | 2-1 | R14 (Egress の S3 録画) | ✅ 完了 (`lambda.ts` が `stream` と `file` を併用、実機で mp4 を確認)                    |
> | 2-2 | P-02 (DESIGN.md 反映)   | ✅ 完了 (2026-09-18。6 項目すべて反映済み)                                               |
> | 2-3 | P-04 (R7 統合テスト CI) | 未着手。AWS + YouTube が要るのでデプロイ環境とセット                                     |
> | 2-4 | R18 (365 日 24h 配信)   | 未着手。DESIGN.md N-1 と矛盾するため ADR 起票が先                                        |
> | 2-5 | Dependabot (P1/P2)      | ✅ 解消済み。オープン PR は 0 件。**@types/node は 24 系に固定** (P2 の推奨は覆っている) |
>
> §1 の「動作中の構成」と §4 の罠は今も有効。§3 の自走プロンプトは §2 を指しているので、
> そのまま貼らずに `NEXT_WORK.md` の「推奨着手順」から拾うこと。
...
### 2-2. P-02: DESIGN.md に KVS WebRTC TURN 運用反映 ✅ 完了 (2026-09-18)

下の 6 項目はすべて DESIGN.md に反映済み。あわせて、反映の過程で見つかった
**実装と食い違う記述も直した** (NLB → Caddy サイドカー / ElastiCache → SFU sidecar の Valkey /
発表者状態の保存先は Valkey ではなく DynamoDB + room metadata)。
...
### 2-5. Dependabot PR 整理 (P1 / P2) ✅ 解消済み

- ~~**PR #8**: vite 5.4.21 → 8.0.16~~ → 2026-06-20 にマージ済み
- ~~**PR #7**: @types/node 24.13.2 → 25.9.3~~ → マージ後 `ab3bd75` で**意図的に 24 系へ戻した**。
  Lambda / Fargate のランタイムが Node 24 なので型定義も 24 に揃える (CLAUDE.md 参照)。**上げないこと**
...
- deploy 必要時は `cdk deploy StagecastControlPlane -c mediaHostedZoneName=aws.example.com -c budgetMonthlyUsd=30 -c budgetEmail=ops@example.com -c initialAdmins=ops@example.com --require-approval never` を使う (context 省略すると既存リソース消える diff、 注意)
...
console.log("URL=https://dXXXXXXXX.cloudfront.net/join?token=" + encodeURIComponent(payloadB64 + "." + sig));
'
```
