...
- [ ] 各アカウント × 主要リージョン (ap-northeast-1, us-east-1) で `cdk bootstrap`
      **dev は両リージョンとも完了済み** (2026-09-19 に `StagecastWebCertificate` を
      us-east-1 へデプロイできたことで確認)。残りは staging / prod。
      ADR 0028 以降 us-east-1 は必須 (Web の ACM 証明書スタックの置き場所)。
  ```bash
...
Tier 0 として確認済みのずれは次の 5 件:

- **「配信終了」ボタンが `controller.leave()` しか呼ばない** (`App.tsx:1104-1109`)。
  ダイアログは「メディアスタックを破棄します」と言うが API 呼び出しは 0 件。
  stage-web が持つ資格情報はモデレーター相当の招待トークンだけなので、本当に終了させるには
  ADR が要る (U-1 / U-14)
- **Moderator サブビューに `EgressControl` が無い**。ADR 0026 D-3 と `stage-client.ts` は
  モデレーターを通しているのに、描画は Admin だけ (U-2)
- **「LIVE」「ON AIR」が `egressActive` を見ていない**。room に繋がれば全ロールで点灯する (U-3)
- **Admin の「再試行」経路だけ `applyRoomMetadata` を呼ばない** (`App.tsx:639-664`)。
  再試行で繋いだ管理者は grid / idle の初期表示で始まる (U-4)
- **`LiveStats` の 5 指標中 3 つが常に「-」** (`App.tsx:1042-1045`) (U-5)

未起票だった UX 課題 (グローバルショートカット無し、字幕 UI ゼロ、レスポンシブ無し) も同じ計画に
Tier 1 / Tier 2 として載せた。字幕は経路が無いので ADR 0030 が先。
...
### D18. イベントの全件読みが 2 経路ある (ページングなし)

`GET /events` は `EventRepository.list()` の結果をそのまま返し、admin-web も全件を受け取って
カレンダーに描く。ADR 0024 で保持ポリシーを「終了済みだけ削る」ソフトキャップにしたため、
イベントの総数は `1000 + 未終了の件数` まで伸びる。件数に比例してレスポンスサイズと RCU が
伸びるので、体感で重くなったら一覧 API のページング (または「終了済みは別クエリ」への分割) が必要。

**もう 1 経路が `trimOldEvents`**。`create` のたびに `list()` で全件を読むので、イベント作成
1 回あたりテーブル全体の Query (1MB ごとに 1 ラウンドトリップ) を払う。ADR 0024 で
`LastEvaluatedKey` ループを入れるまでは、1MB 打ち切りが偶然このコストを抑えていた。
削除の要否判定に必要なのは件数だけなので、`Select: "COUNT"` + `status = ended` の
FilterExpression に置き換えれば転送量は消える (RCU は変わらない)。

同じページング漏れが `DynamoEventRequestRepository.list()` (`services/control-api/src/repo/dynamo.ts`)
にも残っている。イベント側は ADR 0024 で直したが、リクエスト側は件数が伸びにくいため未対応のまま。
...
- 残 2: **メディア層のアラームは購読する手段が無い**。`event-media-stack.ts` は
  **イベントごとに新しい SNS Topic を作る**ので、コメントの「運用者が後で email/Slack を
  ...
  CaptionLatency / RtmpDisconnect / SinkError / TranslateError の 5 種が全部この状態。

  直すなら **per-event トピックをやめ、制御層の長寿命トピックに publish する**のが筋。
  RenderTemplateFunction は既に env を受け取っているので ARN を渡すのは容易だが、
  **トピック側のリソースポリシーで `cloudwatch.amazonaws.com` からの publish を
  許可する必要がある**。ここを間違えるとスタック作成が失敗し、**配信が始められなくなる**
  (D15/D16 と同じ失敗クラス) ので、実機確認とセットで進めること。

- 残 3: **`budgetEmail` が未設定**。仕組みはあるが設定されていないため、
  月額 50 USD の 80% / 100% 通知も**誰にも届いていない** (2026-09-17 時点で購読者ゼロを確認)。
  `infra/user-config.ts` に書くだけ (L3 とセットで対応)
...
> R1〜R2・R4〜R6・R8〜R12・R14〜R17 は完了 (R6 の ACM も ADR 0028 で完了)なので、着手順も現在地に合わせる。

1. **D8 残 1**: control-api の LiveKit 呼び出しの再試行。ADR 0026 で `egressId` が
   サーバに乗ったので、二重 Egress を避けつつ包める
2. **R7**: 統合テスト CI workflow。異常系が実機でしか分からない状態が ADR 0027 / 0028 の
   周辺と全部同じ失敗クラスなので、ここが一番効く
3. **O1 / O2 / O5**: AWS 認証・GitHub Environment・Secrets の実値。手元運用から
   「他人が deploy できる」状態にするときに必要
4. **L1**: 利用規約 / プライバシーポリシー。公開配信の前に必ず
5. **N3 残 2**: メディア層のアラームが誰にも届かない件。イベントごとに SNS Topic を作る設計のまま
6. **D / N の残り**は R を進めながら **思い出した時に PR を切る** のが現実的
