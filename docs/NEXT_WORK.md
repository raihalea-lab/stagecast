...
11. **D21: 配信画面 (stage-web) の文言と実装のずれ** → 計画は [`STAGE_UX_PLAN.md`](./STAGE_UX_PLAN.md)。
    ~~Tier 0 (5 件、`App.tsx` 中心) を 1 PR で~~ ✅ 2026-09-24 実装 (PR #289)。残りは Tier 1 (U-6〜U-12)
...
> ✅ **Tier 0 の 5 件は 2026-09-24 に実装 (PR #289)**。「配信終了」ボタンは消して管理画面への案内に置き換え、
...
2. ~~**D21 Tier 0**: 配信画面の「配信終了」が API を呼ばない件と、送出状態と無関係に点く
   「ON AIR」~~ ✅ 2026-09-24 実装 (PR #289)。次は Tier 1 の U-6 (ショートカット) と U-11 (起動段階の表示)
