# ライブ配信プラットフォーム 設計ドキュメント

StreamYard 型 YouTube ライブ配信 Web アプリケーション
AWS / イベント単位スケーリング / 多言語リアルタイム字幕
バージョン 1.0

---

## 1. 概要

StreamYard に相当する YouTube ライブ配信用 Web アプリケーション。配信管理者・イベントモデレーター・登壇者が共同で配信を運営し、スライド投影と登壇者映像の合成、発表者の切り替え制御、QR コードやイベントタイトルのオーバーレイ表示、日本語と英語を中心としたリアルタイム字幕翻訳を行う。最終的な映像と字幕は YouTube Live を通じて一般視聴者へ配信される。

システムは AWS 上に構築する。費用最適化を重視し、配信を行っていない期間はメディア処理層と翻訳層をゼロスケールさせ、常時稼働するのは静的配信・API・データストアといった低コストな制御層のみとする。配信単位でメディア処理リソースをオートスケールさせ、現実的な見込みとして最大 3 つの別イベントを同時並行で配信できる想定だが、アーキテクチャ上の上限ではない (ADR 0008)。

### 1.1 用語

| 用語          | 意味                                                                                                     |
| ------------- | -------------------------------------------------------------------------------------------------------- |
| SFU           | Selective Forwarding Unit。WebRTC の映像・音声を参加者間で中継するメディアサーバー。LiveKit を採用する。 |
| Egress        | SFU 上の複数トラックを 1 本の映像に合成し、RTMP で外部へ送出する処理。                                   |
| 字幕イベント  | 音声から生成される、開始時刻・終了時刻・言語・テキスト・確定フラグを持つ最小単位のデータ。               |
| 字幕バス      | 各エンジンが生成した字幕イベントを共通形式で受け取り、各出力先へ配る内部メッセージ経路。                 |
| 出力先 (Sink) | 字幕バスを購読し、YouTube や独自 API など特定の宛先へ字幕を送る実装。                                    |
| ゼロスケール  | リソースを 0 インスタンスまで縮退させ、利用していない間の課金を最小化すること。                          |

---

## 2. 要件

### 2.1 機能要件

| ID   | 要件                                                                                                             |
| ---- | ---------------------------------------------------------------------------------------------------------------- |
| F-1  | 配信管理者（約2名）、イベントモデレーター（約3名）、登壇者（約8名）が同一イベントに参加できる。                  |
| F-2  | スライドの投影と登壇者映像を 1 つの配信画面に同時に表示できる。                                                  |
| F-3  | スライド投影は、登壇者の画面共有と、事前アップロードした資料をアプリ側で送る方式の両方に対応する。               |
| F-4  | 管理者が、発表中の登壇者とそうでない登壇者を制御できる（画面への出し入れ）。                                     |
| F-5  | QR コードおよびイベントタイトルを配信画面に表示できる。                                                          |
| F-6  | 配信は YouTube Live で行い、一般視聴者は YouTube から視聴できる。                                                |
| F-7  | 登壇者音声のリアルタイム字幕翻訳を行う。最低限、日本語と英語に対応する。                                         |
| F-8  | ASR / 翻訳エンジンを複数の選択肢から差し替えられる（LLM 経路、Amazon Transcribe Streaming 経路など）。           |
| F-9  | 別イベントを同時並行で配信できる (現実的な見込みは最大 3、上限は AWS quota と運用設定の soft cap ADR 0008 D-6)。 |
| F-10 | イベントごとに QR コード・配信素材・各種設定を準備・登録できる。                                                 |
| F-11 | 確定字幕を配信プロトコルとして外部へ提供できる（独自字幕配信 API）。                                             |
| F-12 | 管理者は Cognito で認証する。モデレーターと登壇者は招待 URL でアクセスする。                                     |

### 2.2 非機能要件

| ID  | 要件                                                                             |
| --- | -------------------------------------------------------------------------------- |
| N-1 | 配信を行っていない期間は、メディア層・翻訳層をゼロスケールし、課金を最小化する。 |
| N-2 | 字幕の遅延は 3 秒以内を目標とする。                                              |
| N-3 | サーバーが必要な場合は AWS を利用する。                                          |
| N-4 | 配信録画および確定字幕は S3 に保存する。                                         |
| N-5 | イベント単位でメディア処理リソースをオートスケールし、イベント間で干渉しない。   |

### 2.3 字幕出力に関する前提

YouTube Live はリアルタイム字幕として 1 トラックのみを受け付ける。このため、常用経路では確定字幕を 1 言語だけ YouTube へ送出する。複数言語を同時に提供したい場合に備えて、独自の字幕配信プロトコル（API）を別経路として用意する。独自 API は普段の YouTube 配信では使用せず、イベント設定で有効化したときのみ起動する。提供範囲はプロトコル（配信エンドポイントの仕様）のみとし、視聴ページや埋め込み部品は利用側で実装する。

---

## 3. 全体アーキテクチャ

3 つの層に分かれる。常時稼働する制御層、イベント時のみ起動するメディア層、字幕を生成・配信する翻訳・字幕層。制御層以外は配信が行われていない間はゼロスケールする。

### 3.1 制御層（常時稼働・低コスト）

- **S3 + CloudFront**: SPA の静的ホスティングと CDN 配信。admin-web (管理画面) / stage-web (登壇者・モデレーター) / composer-web (合成テンプレート、ADR 0012 D-2) / request-web (イベント開催申請) の 4 つを**独立した Distribution** で配る。
- **API Gateway + Lambda**: イベント設定 API、発表者制御 API、メディア層の起動・停止のオーケストレーション。リクエスト課金のため非配信時はほぼ無料。
- **DynamoDB**: イベント定義、参加者・ロール、招待トークン、発表状態などのメタデータ。
- **S3（素材・成果物）**: QR コード画像、スライド資料、配信素材、配信録画、確定字幕ファイル。
- **Cognito**: 管理者の認証。モデレーター・登壇者は署名付き招待 URL でアクセス。

### 3.2 メディア層（イベント時のみ起動・並列実行）

イベント開始時に配信単位で独立したメディア処理スタックを起動し、終了時に破棄する。並列数の現実的な見込みは最大 3 だが、アーキテクチャ上は per-event で独立しているため `MAX_PARALLEL_EVENTS` (soft cap) と AWS quota の範囲で並列実行できる (ADR 0008)。

- **SFU (LiveKit)**: 登壇者・モデレーター・管理者の WebRTC 映像音声を中継。画面共有もここで扱う。 シグナリング (7880) の TLS 終端は **SFU Task に同居する Caddy サイドカー**が行い、 Let's Encrypt の DNS-01 チャレンジで証明書を自動取得して S3 に永続化する (ADR 0016 D-1/D-6)。 `wss://event-{slug}.media.{domain}` で per-event の signaling URL を払い出し、 Fargate Public IP への A レコードは reconcile Lambda が UPSERT/DELETE する (ADR 0016 D-3)。 ~~ADR 0009 の NLB + ACM~~ は idle で月 ~$16 かかり N-1 と矛盾するため廃止した。
- **合成・Egress**: カスタム composer-template (ADR 0012) を Chrome ヘッドレスで描画し、 RTMP で YouTube Live + S3 に録画 mp4 を送出する。 Egress は SFU と同一 ECS Task の sidecar として同居 (ADR 0010) し、 SFU には `ws://localhost:7880` で繋ぐ (Chrome の LNA 制限回避のため `insecure: true` 必須、 ADR 0010 D-7)。
- **TURN (AWS KVS WebRTC)**: シンメトリック NAT 越え用。 制御層の `KVS Signaling Channel` (常時稼働、 月 $0.03) が短期 credential 付き iceServers を control-api `/join` 経由で配り、 stage-web は `Room.connect` の `rtcConfig.iceServers` に直接渡す (ADR 0011 案 E)。 LiveKit Server の内蔵 TURN や coturn sidecar は採用しない。
- **Valkey (SFU Task の sidecar)**: LiveKit の psrpc レジストリ (SFU ↔ Egress の相互発見) を担う。 ADR 0017 D-1 で **ElastiCache も独立 Fargate Service も廃止**し、同一 Task 内の `localhost:6379` で繋ぐ (CloudMap DNS も不要になった)。 **投影・レイアウト・発表者の状態はここには置かない** (下記 3.4 / 5.3 を参照)。

### 3.3 翻訳・字幕層（イベント時のみ起動）

- **字幕パイプライン**: 登壇者音声を分岐して受け取り、ASR と翻訳を行い、字幕イベントを生成する。エンジンは差し替え可能。
- **字幕バス**: 生成された字幕イベントを共通形式で集約し、各出力先へ配る。
- **出力先**: YouTube 字幕トラック（1言語・常用）と、独自字幕配信 API（多言語・任意起動）。

### 3.4 データフロー

1. 登壇者・モデレーター・管理者が招待 URL から `POST /join` を叩き、LiveKit token と
   **KVS WebRTC の短期 iceServers** を受け取る。ブラウザは `Room.connect` の
   `rtcConfig.iceServers` にそれを直接渡して SFU に接続し、映像音声・画面共有を送る (ADR 0011 案 E)。
2. 管理者・モデレーターが発表者の出し入れやレイアウトを操作すると、制御 API が
   **DynamoDB の `PresentationState` を更新** し、続けて **LiveKit の room metadata を書き換える**。
   合成処理 (composer) と stage-web の全ウィンドウは metadata の変更を受け取って即座に追従する
   (ADR 0022 / ADR 0025 / ADR 0026)。
3. 合成処理がスライド・登壇者・QR・タイトルを 1 枚の映像に合成し、RTMP で YouTube Live へ送出する。
4. 登壇者音声は字幕パイプラインにも分岐され、ASR・翻訳を経て字幕イベントになる。
5. 字幕イベントは字幕バスに集約され、YouTube 字幕トラックへ（1言語）、必要に応じて独自 API へ（多言語）配られる。
6. 一般視聴者は YouTube で映像と字幕を視聴する。録画と確定字幕は S3 に保存される。

---

## 4. 参加者・ロール・認証

| ロール               | 想定人数 | 認証方式 | 主な権限                                                         |
| -------------------- | -------- | -------- | ---------------------------------------------------------------- |
| 配信管理者           | 約2名    | Cognito  | イベント設定、メディア層の起動・停止、発表者の出し入れ、全体制御 |
| イベントモデレーター | 約3名    | 招待URL  | 進行補助、登壇者の案内、チャット・Q&A の管理                     |
| 登壇者               | 約8名    | 招待URL  | 自分の映像音声・画面共有の送信、スライド送り                     |
| 一般視聴者           | 無制限   | 不要     | YouTube での視聴、字幕言語の選択                                 |

### 4.1 招待 URL の仕組み

モデレーターと登壇者にはアカウントを発行せず、イベント単位の署名付き招待 URL を共有する。URL にはイベント ID・ロール・有効期限を含むトークンを埋め込み、サーバー側で署名検証する。

- トークンはイベントごと・ロールごとに発行し、失効・再発行が可能。
- 有効期限はイベント開催時間に合わせて設定する。
- 管理者のみ Cognito による恒久的なアカウントを持つ。

### 4.2 制御 API のエンドポイント (主要)

招待トークンで入るモデレーター・登壇者は Cognito のアカウントを持たないため、該当ルートは
**API Gateway の JWT authorizer を通さず** control-api 内でトークンを検証する。

| Path                                                  | 認証         | 用途                                                                                                                   |
| ----------------------------------------------------- | ------------ | ---------------------------------------------------------------------------------------------------------------------- |
| `POST /join`                                          | 招待トークン | stage-web の入室 (LiveKit token 発行)。 KVS WebRTC TURN の iceServers を含めて返す (ADR 0011)                          |
| `POST /invites/verify`                                | 招待トークン | invite token の事前検証 (UX 補助)                                                                                      |
| `POST /preview-token`                                 | 招待トークン | stage-web の登壇者ビュー右下小窓プレビュー (viewer role、 ADR 0012 D-6)                                                |
| `POST /stage/presentation/{state,slide,layout}`       | 招待トークン | 投影・スライド送り・レイアウトの更新。`PresentationState` を書き、room metadata に反映 (ADR 0022 / 0025)               |
| `POST /stage/egress/{start,stop}`                     | 招待トークン | モデレーターによる YouTube 送出の開始・停止 (ADR 0026 D-4)。モデレーター以外は 403                                     |
| `POST /stage/{assets,materials,presets}/*`            | 招待トークン | デッキ・素材の署名付き URL 発行とプリセット操作                                                                        |
| `POST /event-requests` / `GET /event-requests/public` | なし         | request-web からのイベント開催申請と、公開カレンダー用の一覧                                                           |
| `GET /events/public`                                  | なし         | 公開イベント一覧                                                                                                       |
| `POST /events/{id}/admin-token`                       | Cognito      | admin-web の LayoutControl が LiveKit room に admin role で接続し、 data channel で layout を broadcast (ADR 0012 D-4) |
| `POST /events/{id}/stage-token`                       | Cognito      | 管理者が stage-web に入るための token (ADR 0014 D-4)                                                                   |
| `POST /events/{id}/preview-token`                     | Cognito      | admin-web の LivePreview iframe (viewer role)                                                                          |
| `POST /events/{id}/egress/start`                      | Cognito      | 管理側からの YouTube Live RTMP 送出 + S3 録画開始 (ADR 0006 D-4)                                                       |
| `GET /events/{id}/artifacts`                          | Cognito      | 配信成果物 (録画 / 確定字幕) のダウンロード URL 一覧                                                                   |
| その他 `/events/*`                                    | Cognito      | events CRUD / status 遷移 / invites / settings 等                                                                      |

> **公開ルートの正は `packages/shared/public-routes.json`**。infra はこれを回して `CfnRoute` を
> 生成する。ここへの書き忘れは `$default` の JWT authorizer に弾かれ、**Lambda に届く前に 401**
> になる (実際に 4 回踏んでいる)。ワイルドカードにはしない — 将来の管理者パスまで素通りする。

---

## 5. 配信画面の合成

配信画面は、登壇者映像・スライド・オーバーレイ要素を 1 本の映像に合成して生成する。合成は SFU 上のトラックを Egress が受け取り、 ADR 0012 のカスタム composer-template (React app) に渡してレイアウトを適用、 そのまま RTMP + S3 録画に出力する。

### 5.1 合成要素

- **登壇者映像**: 発表中としてマークされた登壇者を表示。複数名同時表示にも対応。
- **スライド (画面共有)**: 登壇者のブラウザから WebRTC 画面共有として publish。 同一 participant のカメラと画面共有は composer-template 上で別 tile として並列表示される (StreamYard 風)。
- **QR コード**: イベント設定で登録した QR をオーバーレイ表示 (将来カスタムテンプレートに組み込み予定)。
- **イベントタイトル**: 下部または上部のテロップとして表示 (同上)。

### 5.1.1 カスタム合成テンプレート (composer-template, ADR 0012)

LiveKit Egress のデフォルトテンプレートではなく、 自前の React app (`apps/composer-template/`) を S3 + CloudFront でホストし、 Egress config の `template_base` で指定して使う。 これにより以下を実現:

| 機能                                                       | 仕組み                                                                                                                                        |
| ---------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------- |
| 4 種の layout (grid / spotlight / pip / screen-share-main) | テンプレート内 React state で切替 (ADR 0012 D-4)                                                                                              |
| layout 操作                                                | admin-web の LayoutControl が LiveKit data channel で broadcast → テンプレート側 `RoomEvent.DataReceived` で受信 → 即時 (sub-second) 切替     |
| 待機画面                                                   | publishing participant 0 人時は `<WaitingScreen />` を表示 (要件 3、 ADR 0012 D-5)                                                            |
| プレビュー                                                 | admin-web の LivePreview と stage-web の登壇者ビューに **同じテンプレートを iframe 埋め込み** で再利用 (subscriber-only token、 ADR 0012 D-6) |
| Egress プロトコル                                          | テンプレートは Room.connect 成功時に `console.log("START_RECORDING")` を発行して Egress に描画開始を通知 (R15-followup-1)                     |

### 5.2 スライド投影の二方式

| 方式             | 内容                                                                                              |
| ---------------- | ------------------------------------------------------------------------------------------------- |
| 画面共有         | 登壇者のブラウザから WebRTC の画面共有トラックを送る。任意のアプリの画面を共有できる。            |
| 事前アップロード | PDF・画像を事前に S3 へ登録し、アプリ側でページを送る。共有が不安定な環境でも安定して投影できる。 |

### 5.3 発表者の制御

管理者は管理画面から、各登壇者を「発表中」「待機」に切り替える。

**状態の正は DynamoDB の `PresentationState`** (発表者・スライド・レイアウト・主役)。
そこから LiveKit の **room metadata** に写して配る (ADR 0022 D-1 / ADR 0025 D-2)。
composer は Egress が起動する headless Chrome の中にいて token と url しか受け取らないため
制御 API を叩けないが、metadata なら**接続時に必ず届き**、更新も `RoomMetadataChanged` で拾える。
これにより composer は「配り直されるのを待つ」側から「状態を読む」側になり、再接続しても
レイアウトや投影が巻き戻らない。YouTube へ送出中かどうか (`egressActive`) も同じ経路で配る
(ADR 0026 D-2)。

DataChannel による broadcast は「今いる人」にしか届かないので、**正としては使わない**。

---

## 6. リアルタイム字幕パイプライン

字幕機能の核は、字幕を共通形式の「字幕イベント」として生成し、変換エンジンと出力先の双方を差し替え可能にする抽象化である。エンジンも出力先も同じ字幕バスを介するため、組み合わせは自由で、後からの追加も容易である。

### 6.1 字幕イベントの共通形式

- 開始時刻・終了時刻（メディアタイムライン基準）
- 言語コード（例: ja, en）
- テキスト
- 確定フラグ（暫定 / 確定）
- 話者・トラック識別子（任意）

### 6.2 差し替え可能なエンジン層

ASR と翻訳のエンジンは共通インターフェースを実装し、イベント設定で選択する。経路ごとに異なるエンジンを割り当てることもできる。

| エンジン                    | 特性                                                                     | 主な用途         |
| --------------------------- | ------------------------------------------------------------------------ | ---------------- |
| Amazon Transcribe Streaming | 低遅延・安定。ストリーミング ASR。翻訳は Amazon Translate と組み合わせ。 | 常用・低遅延経路 |
| LLM 経路                    | ASR+翻訳または翻訳のみ。文脈を考慮し品質が高いが遅延は増えやすい。       | 品質重視経路     |
| 自前 ASR                    | GPU 上で任意モデルを実行。将来的な拡張・特殊要件向け。                   | 拡張用           |

遅延目標 3 秒以内は Transcribe Streaming + Amazon Translate で現実的に達成できる。LLM 経路は品質に優れるが遅延が増えやすいため、YouTube 字幕トラックには Transcribe 経路、品質を優先したい独自 API の一部言語には LLM 経路を割り当てる。

### 6.3 出力先（Sink）

**6.3.1 YouTube 字幕トラック（常用・1言語）**
YouTube Live はリアルタイム字幕として 1 トラックのみ受け付けるため、確定字幕を 1 言語だけ送出する。視聴者はプレーヤー上で字幕の表示・非表示を切り替えられる。

**6.3.2 独自字幕配信 API（多言語・任意起動）**
複数言語を同時に提供する場合に備え、独自の字幕配信プロトコルを提供する。提供範囲はプロトコル（配信エンドポイントの仕様）のみとし、視聴ページや埋め込み部品は利用側で実装する。普段の YouTube 配信では使用しないため、イベント設定で有効化したときのみ起動し、それ以外はゼロスケールする。

- 字幕バスを購読し、確定・暫定の字幕イベントを全対応言語ぶん配信する。
- WebSocket / SSE などのストリーミング配信を想定。仕様とドキュメントを提供し、ページ実装は利用側に委ねる。

### 6.4 字幕の保存

確定字幕は S3 に保存し、イベント後にトランスクリプトや字幕ファイル（SRT/VTT）として利用できるようにする。配信録画も同様に S3 に保存する。

---

## 7. イベント単位のスケーリングとコスト最適化

メディア層・翻訳層は配信単位で独立して起動・破棄する。複数の別イベントが同時並行する場合、それぞれに独立した SFU・Egress・字幕パイプラインのセットを割り当て、相互に干渉しないようにする。並列数は per-event で独立した構成となっており、`MAX_PARALLEL_EVENTS` の soft cap と AWS quota が上限になる (ADR 0008)。

### 7.1 ライフサイクル

1. イベントが `scheduled` になった時点で、スタックだけを **desiredCount 0 で先に作っておく**
   (リソースは無料のまま。ADR 0016 D-4)。開始操作ではタスクを 0→1 に引き上げるだけで済む。
2. 管理者がイベントを開始すると、制御層（Lambda）がそのイベント専用のメディアスタックと字幕パイプラインを起動する。
3. **「配信できる」の判定は ECS が RUNNING を返したことでは足りない** (ADR 0027)。
   コンテナ起動の後に LiveKit の初期化・Caddy の証明書ロード・Route53 の DNS 反映が続くため、
   実際にシグナリングを叩いて応答があることまで確認して `ready` を出す。
   ここを RUNNING だけで判定していたとき、管理画面が `ready` のままで誰も入室できない事故が起きた。
4. 配信中はイベントごとに独立したリソースで処理する。並列数は `MAX_PARALLEL_EVENTS` (デフォルト 10) と AWS quota の範囲。
5. 管理者がイベントを終了すると、当該スタックを破棄する。
6. 非配信時に稼働するのは制御層（S3/CloudFront・API Gateway/Lambda・DynamoDB・S3・Cognito）と
   TURN 用の KVS Signaling Channel (月 $0.03) のみ。

### 7.2 コスト最小化の方針

| コンポーネント                   | コスト方針                                                                                                                                                                                                                |
| -------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| S3 + CloudFront (SPA 4 つ)       | 静的配信。 アクセス量に応じた低コスト。 admin / stage / composer / request をそれぞれ独立 Distribution で配る (ADR 0012 D-2)。                                                                                            |
| API Gateway + Lambda             | リクエスト課金。 非配信時はほぼ無料。                                                                                                                                                                                     |
| DynamoDB                         | オンデマンド課金。 メタデータのみで小規模。                                                                                                                                                                               |
| Valkey (SFU Task の sidecar)     | **専用リソースなし**。 ADR 0017 D-1 で ElastiCache と独立 Fargate Service を廃止し、 SFU Task に同居させた。                                                                                                              |
| SFU / Egress / Valkey / 字幕     | イベント時のみ起動し終了で破棄。 SFU + Egress + Valkey + Caddy は同一 Fargate Task に同居 (ADR 0010 / 0016 / 0017)。 字幕が不要なイベントでは CaptionWorker を起動しない (ADR 0017 D-2、 -35%)。 非配信時はゼロスケール。 |
| 独自字幕 API                     | イベント設定で有効化した時のみ起動。 普段は休止。                                                                                                                                                                         |
| **KVS Signaling Channel (TURN)** | **常時稼働** だが約 **$0.03/月** + 利用量 (TURN streaming) 課金。 stage-web の NAT 越え用 (ADR 0011 案 E)。                                                                                                               |

### 7.3 並列配信の独立性

別イベントはそれぞれ別の管理者・モデレーター・登壇者が参加する独立した配信。SFU・Egress・字幕パイプラインをイベント単位のセットとして起動するため、あるイベントの負荷が他へ波及しない。各セットは個別の YouTube Live 配信先へ送出する。per-event の URL ルーティング (`/join` が events.media.livekitUrl を返す) で、複数イベントを跨いでクライアントが取り違える事故を防ぐ (ADR 0008)。

---

## 8. イベントごとの準備・設定

管理者は配信前にイベント単位で必要な素材と設定を準備・登録する。設定は DynamoDB に、素材は S3 に保存する。

- イベントタイトル、開催日時。
- QR コード画像（オーバーレイ表示用）。
- 配信素材（背景、ロゴ、テロップなど）。
- スライド資料（事前アップロード方式を使う場合）。
- 登壇者・モデレーターの招待 URL の発行。
- 字幕の対応言語、YouTube へ送出する言語、エンジンの選択。
- 独自字幕配信 API を有効化するか否か。
- YouTube Live の配信先設定。

### 8.1 運用上の監視ポイント

| 観点                  | 何を見るか                                                               | どこで                                                                 |
| --------------------- | ------------------------------------------------------------------------ | ---------------------------------------------------------------------- |
| Egress 起動失敗       | `egress.failed` ログ + Chrome の hung (stats/monitor が止まる)           | EventMediaStack の CloudWatch Logs / Egress container stream           |
| 待機画面 → grid 切替  | `START_RECORDING` シグナル / publisher 数                                | composer-template の Chrome console (要 ECS Exec で `/tmp/chrome.log`) |
| TURN 取得失敗         | `/join` レスポンスの `iceServers` 欠落 / `GetIceServerConfig` API エラー | control-api Lambda の CloudWatch Logs                                  |
| KVS Signaling Channel | ステータスが ACTIVE か / TURN streaming 使用量                           | AWS Console (Kinesis Video Streams) + Budgets アラート                 |
| ICE pair 失敗         | stage-web 入室直後の WebRTC `state: failed` (15s で SIGNAL_SOURCE_CLOSE) | LiveKit Server (SFU) ログ + Chrome `webrtc-internals`                  |
| 証明書の更新失敗      | `tls.renew` / `tls.obtain` の失敗が 5 分おきに出ていないか               | EventMediaStack の CloudWatch Logs / Caddy container stream            |
| 配信開始できない      | `phase` が `ready` にならない / シグナリングが応答しない (ADR 0027)      | 管理画面の「配信インフラ」カード + reconcile Lambda のログ             |

ADR 0011 + ADR 0010 + ADR 0012 の経緯と踏破済の罠は `r12-livekit-fargate-gotchas` memory に集約 (11 件)。

---

## 9. AWS 構成まとめ

| 層           | 主なサービス                                                                          | 稼働形態                                                       |
| ------------ | ------------------------------------------------------------------------------------- | -------------------------------------------------------------- |
| 制御層       | S3, CloudFront (admin/stage/composer/request), API Gateway, Lambda, DynamoDB, Cognito | 常時稼働・低コスト                                             |
| TURN         | Kinesis Video Streams Signaling Channel                                               | 常時稼働 (約 $0.03/月、 ADR 0011 案 E)                         |
| 共有状態     | DynamoDB (`PresentationState` が正) + LiveKit room metadata (配布)                    | 制御層と同じ (ADR 0022 / 0025 / 0026)                          |
| メディア層   | SFU (LiveKit) + Egress / Valkey / Caddy sidecar (Fargate 同一 Task)                   | イベント時のみ起動・破棄、並列実行可 (ADR 0008/0010/0016/0017) |
| 翻訳・字幕層 | 字幕パイプライン, 字幕バス, 各エンジン・出力先                                        | イベント時のみ起動                                             |
| 保存         | S3（録画 mp4 / 確定字幕 / 素材）                                                      | 常時                                                           |
| 配信         | YouTube Live                                                                          | 外部                                                           |

### 9.1 実装フェーズでの詳細設計事項

- メディアスタックの具体的なコンピュート選択（Fargate / GPU を要する自前 ASR を使う場合は EC2 GPU など）と起動・破棄のオーケストレーション詳細。
- 字幕バスの実装方式（メッセージング基盤）と暫定・確定字幕の同期方式。
- 独自字幕配信 API のプロトコル詳細仕様（メッセージスキーマ、再接続、認証）。
- YouTube Live API 連携（配信開始・字幕送出）の詳細。
- 障害時のフェイルオーバーと、配信途中のリソース再起動方針。
